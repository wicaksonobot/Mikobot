...
let handler = async (m, { conn, args, text, usedPrefix, command }) => {
	if (!text) throw `[!] Input Query\n\n*Example :* ${usedPrefix + command} Gotoubun no Hanayome`
	try {
		if (text.includes('https://kitsu.io')) {
			...
			txt += `\n*Synopsis :*\n"${anu.synopsis}"`
			await conn.sendMessage(m.chat, { image: { url: anu.posterImage.medium }, caption: txt }, { quoted: m })
		} else {
			let response = await fetch(`https://kitsu.io/api/edge/anime?filter[text]=${encodeURIComponent(text)}`)
			...
			anu.data.forEach(function(i) {
				array.push({
					...
					description: `${i.attributes.synopsis.length > 60 ? i.attributes.synopsis.slice(0, 60) + '...' : i.attributes.synopsis}`
				});
			});
			const sections = [
				{
