...
let handler = async (m, { conn, usedPrefix, text, args, command }) => {
  ...
  const sentMsg = await conn.sendContactArray(m.chat, [
    [`${nomorown}`, `${await conn.getName(nomorown+'@s.whatsapp.net')}`, `👑 Developer Bot `, `🚫 Don't call me 🥺`, `Caubong202@gmail.com`, `🇮🇩 Indonesia`, `🚀 https://lynk.id/cahyo_kun`, `👤 Gada pawang nih senggol dong 😔`],
    [`${conn.user.jid.split('@')[0]}`, `${await conn.getName(conn.user.jid)}`, `🔥 Bot WhatsApp 🐣`, `📵 Don't spam/call me 😢`, `Nothing`, `🇮🇩 Indonesia`, `🚀 https://lynk.id/cahyo_kun`, `🤖 Hanya bot biasa yang kadang suka eror ☺`]
  ], m)
