...
if (db.data.users[m.sender].premiumTime < 1) return m.reply('FITUR GAME INI KHUSUS USER PREMIUM !')
const daki = global.akinator 
const shp = "•"
...
if (!args[0]) {
conn.send3ButtonImg(m.chat, 'https://telegra.ph/file/cdc1f976cffcb98cf8033.jpg', akin, footer, 'Start', '.akinator start', 'Stop', '.akinator stop',  'Game Session', '.akinator mysession', m)
}
//start akinator
if (command) {
 switch(template) {
		case 'start': {
			if(daki[m.sender]){
        		...
        		await conn.sendButton(m.chat, foundp, footer, [['Game Session', '.akinator mysession']], m)
			}
			else {
		...
		case 'stop': {
        	...
        	await conn.sendButton(m.chat, foundp, footer, [['Start', '.akinator start']], m)
...
• *Pseudo :* ${res.pseudo}
• *Nsfw :* ${res.nsfw}`
	      				await conn.sendButtonImg(m.chat, ppres, anuu, 'Klik tombol dibawah untuk memulai kembali', 'Start Again', '.akinator start', m)
	      			}catch{
